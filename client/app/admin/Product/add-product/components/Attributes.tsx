...
const Attributes: React.FC<AttributesProps> = ({ onAttributesChange }) => {
  ...
  const [attributes, setAttributes] = useState<
    Array<{ _id: string; name: string; variations: string[] }>
  >([]);
      ...
      try {
        ...
        if (attributesData.length > 0) {
          ...
          attributesData.forEach(
            (attr: { _id: string; name: string; variations: string[] }) => {
  ...
  return (
    <div>
      <h3 className="mb-6 text-[24px] font-semibold break-words text-white">
        Attributes
      </h3>
      <div className="rounded-3xl border border-[#172D6D] bg-black/30 p-6 backdrop-blur-[500px]">
        {loading ? (
          <div className="flex h-32 items-center justify-center">
            <div className="text-lg text-white">Loading attributes...</div>
          </div>
        ) : (
              ...
              <div key={attribute._id}>
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      enabledAttributes[attribute._id]
                        ? "bg-blue-500"
                        : "border border-[#172D6D] bg-gray-300"
                    }`}
                  >
                    <span
                ...
                {enabledAttributes[attribute._id] && (
                  <div className="multi-select-container relative">
                    ...
                    <div
                      className="min-h-[48px] w-full cursor-pointer rounded-lg border border-[#172D6D] bg-black/30 px-4 py-3 text-white transition-colors focus-within:border-[#3B82F6]"
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <svg
                          className={`h-4 w-4 text-white transition-transform ${openDropdowns[attribute._id] ? "rotate-180" : ""}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                    ...
                    {openDropdowns[attribute._id] && (
                      <div className="z-10 mt-1 max-h-48 w-full overflow-y-auto rounded-lg border border-[#172D6D] bg-black/90 shadow-lg backdrop-blur-sm">
