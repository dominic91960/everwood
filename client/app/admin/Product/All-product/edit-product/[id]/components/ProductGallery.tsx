...
const ProductGallery = ({ onFilesChange, existingImages = [] }: ProductGalleryProps) => {
  ...
  return (
    <div>
            ...
            {existingImages.map((image, index) => (
              <div key={index} className="relative">
                <img
                  src={image}
                  alt={`Product ${index + 1}`}
                  className="w-full h-20 object-cover rounded-lg border border-[#6E6E6E]"
                />
              </div>
            ))}
          </div>
      ...
      {/* File Upload Area */}
      <div
        className="border-2 border-dashed rounded-lg border-[#6E6E6E] p-8 text-center hover:border-gray-500 transition-colors cursor-pointer"
        onDrop={handleDrop}
